...
def general_metrics( df1 ):
    ...
    return  registered_restaurants, countries, cities, votes, cuisines

def countries_map( df1 ):
    ...
    with st.container():
        ...
        for index, row in df1.iterrows():
            ...
            # Get the color based on rating_color column
            rating_color = row['rating_color']

            # Create a marker with detailed popup content and colored icon
...
def restaurant_by_country( df1 ):
    ...
    with st.container():
        ...
        fig.update_layout( title='Number of Restaurants by Country', xaxis_title='Country Name',
                          yaxis_title='Number of Restaurants', title_font=dict( size=20 ), height=600 )
        st.plotly_chart( fig, use_container_width=True )

    return fig

def reviews_country( df1 ):
