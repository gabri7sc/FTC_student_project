...
def get_popup_content( row ):
    ...
    return f"""<div style='font-size: 16px; width: 300px;'>
               <b>{row['restaurant_name']}</b><br>
               USD $ Price: {row['dollar_usd_value']}<br>
               Local Currency {row['currency']}<br>
               Local Price {row['average_cost_for_two']}<br>
               Type: {row['cuisines']}<br>
               Aggregate Rating: {row['aggregate_rating']}/5.0
           </div>"""

def headline_metrics( df1 ):
...
def best_worst_classification( df1 ):
    ...
    with st.container():
        ...
        with col1:
            ...
            q21_best = ( df1[['restaurant_name', 'restaurant_id', 'aggregate_rating']]
            .groupby( 'restaurant_name' ).agg( {'restaurant_id': 'first', 'aggregate_rating': 'mean'}) )
...
def top_most_individual_visited_restaurant( df1 ):
...
def top_most_visited_franchise( df1 ):
    with st.container():
...
def cuisine_average_rating( df1 ):
...
with tab1:
    ...
    best_worst_classification( df1 )

    st.markdown( """___""" )

    top_most_individual_visited_restaurant( df1 )

    st.markdown("""___""")

    top_most_visited_franchise( df1 )

with tab2:

    cuisine_distribution(df1)

    st.markdown( """___""")

    cuisine_average_rating( df1 )
    ...
    with st.container():
        ...
        for index, row in df1.iterrows():
            ...
            folium.Marker(location=[row['latitude'], row['longitude']],
                          popup=popup_content,
                          icon=folium.Icon( color=rating_color, icon='cutlery' ) ).add_to( marker_cluster )
